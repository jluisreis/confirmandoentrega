...
function isEntregue(p: Pedido): boolean {
  const logistica = String(p.LOGISTICA ?? '').toUpperCase().trim()
  return Boolean(
    p['ENTREGUE DATA'] ||
      p['ENTREGUE HORA'] ||
      logistica === 'ENTREGUE' ||
...
// 🔥 NOVO: status de LOGISTICA que podem ser filtrados no painel
const STATUS_LOGISTICA_FILTRO = ['AGENDADA', 'ENTREGA', 'VAI HOJE', 'VAI AMANHÃ']
...
export default function DataTable({ sessao }: DataTableProps) {
  ...
  useEffect(() => {
    ...
    function handleClickFora(e: MouseEvent) {
      if (logisticaMenuRef.current && !logisticaMenuRef.current.contains(e.target as Node)) {
        setLogisticaMenuAberto(false)
      }
    }
    document.addEventListener('mousedown', handleClickFora)
    return () => document.removeEventListener('mousedown', handleClickFora)
  ...
  function confirmarDialogo() {
    ...
    if (!responsavelSelecionado) {
      setErroDialog('Selecione o responsável pela entrega.')
      return
    }
    setErroDialog(null)
    if (modoManual) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="space-y-1 relative" ref={logisticaMenuRef}>
          ...
          <button
            type="button"
            onClick={() => setLogisticaMenuAberto((v) => !v)}
            className={`flex items-center gap-2 w-56 px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border text-sm text-left transition-all ${
          ...
          >
            ...
            <span className="flex-1 truncate">
              {logisticaFiltro.length === 0
                ? 'Todos os status'
                : logisticaFiltro.length === 1
                  ? logisticaFiltro[0]
                  : `${logisticaFiltro.length} status selecionados`}
            </span>
            <ChevronDown className={`w-4 h-4 flex-shrink-0 transition-transform ${logisticaMenuAberto ? 'rotate-180' : ''}`} />
          </button>

          {logisticaMenuAberto && (
            <div className="absolute z-10 mt-1 w-56 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-lg">
              <div className="flex items-center justify-between px-3 py-2 border-b border-slate-100 dark:border-slate-800">
                <button
                  type="button"
                  onClick={() => setLogisticaFiltro([...STATUS_LOGISTICA_FILTRO])}
                  className="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline"
                >
                  Selecionar todos
                </button>
              ...
              <ul className="py-1">
                {STATUS_LOGISTICA_FILTRO.map((status) => {
                  const marcado = logisticaFiltro.includes(status)
                  return (
                    <li key={status}>
                      <label className="flex items-center gap-2 px-3 py-1.5 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer">
                        <input
                          ...
                          checked={marcado}
                          onChange={() => {
                            setLogisticaFiltro((atual) =>
                              marcado ? atual.filter((v) => v !== status) : [...atual, status]
                            )
                            setCurrentPage(1)
                          }}
                          className="w-4 h-4 rounded border-slate-300 dark:border-slate-700 text-blue-600 focus:ring-blue-500/30"
      ...
      {dialogPedido && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-5 space-y-4">
            ...
            {/* 🔥 NOVO: responsável pela entrega — só o login ADMINISTRACAO pode trocar */}
            <div className="space-y-1">
              <label className="text-xs font-medium text-slate-600 dark:text-slate-300">
                Responsável pela entrega
              </label>
              {sessao.isAdmin && sessao.responsaveisDisponiveis ? (
                <select
                  value={responsavelSelecionado}
                  onChange={(e) => setResponsavelSelecionado(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-800 dark:text-slate-100"
                >
                  {sessao.responsaveisDisponiveis.map((r) => (
                ...
                <div className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-sm text-slate-600 dark:text-slate-300">
