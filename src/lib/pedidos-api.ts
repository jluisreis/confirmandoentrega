...
export interface LoginResult {
  ...
  responsavel: string;
  isAdmin: boolean;
  /** só vem preenchido pro usuário ADMINISTRACAO */
  responsaveisDisponiveis: string[] | null;
}

export async function login(usuario: string, senha: string): Promise<LoginResult> {
  if (!SCRIPT_URL || !SCRIPT_SECRET) throw new Error("Backend não configurado");
  ...
  if (!json.ok) throw new Error(json.error || "Usuário ou senha inválidos");
